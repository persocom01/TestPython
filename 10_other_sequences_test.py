...
# However, a new list can be generated with sorted values and a new dictionary can be written (or the old one overwritten) using the sorted keys.
...
sort_by_values_but_return_keys = sorted(items, key=items.__getitem__, reverse=False)
...
sort_by_function = sorted(items, key=(lambda x: 0 if 'a' in x else 1), reverse=True)
