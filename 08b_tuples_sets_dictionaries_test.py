...
print('tuple:')
print(tuple)
# Similar to lists, a tuple('string') will turn the string into a list of
# letters. Also not that a single length tuple always has a comma.
tuple = ('first',)
print(tuple)
...
# items = {**items, **items2}
# Note that in both cases, the second dictionary will overwrite the first if
# they share common keys. If this is undesirable, a for loop is recommended
# instead. A for loop for combining lists in a first dictionary with lists in
# a second is as follows:
# for key, value in items.items():
#     if key in items2:
#         items2[key].extend(value)
#     else:
#         items2[key] = value
items.update(items2)
